...
namespace SXJL.GTCTK
{
    /// <summary>
    /// 为了项目只特定于Mk2特定型号用
    /// </summary>
    public class Dpm5MK2Helper
    {
        /// <summary>
        /// 大屏幕是否连接
        /// </summary>
        ...
        /// 大屏幕地址
        /// </summary>
        private readonly string _dpmIpAddress;

        //5MK2的类型为4 控制器类型
        private readonly int type = 4;
        private uint m_dwCurHand;

        //屏幕X
        private readonly string areaX = "0";
        //屏幕Y
        private readonly string areaY = "0";
        //屏幕Widt
        private readonly string areaWidth = "192";
        //屏幕Height
        private readonly string areaHeight = "168";
        //行间距
        private readonly string lines_sizes = "0";
        private Led5kSDK.bx_5k_area_header bx_5k;
        private static byte[] AreaText;
        public Dpm5MK2Helper(string dpmIpAddress)
        {
            //大屏幕初始化
            Led5kSDK.InitSdk(2, 2);
            _dpmIpAddress = dpmIpAddress;
            IsConnect = MK1Connect(dpmIpAddress);
        }

        /// <summary>
        /// 发送消息，约定$为换行符，每行12个汉字，第一行和第二行为时间和日期。第三行开始为内容
        /// </summary>
        /// <param name="txt"></param>
        /// <param name="i"></param>
        /// <returns></returns>
        public bool SendTxt(string txt, out string viewTxt, uint i = 0)
        {
            string sendTxt = FormatStr(txt);
            Log2Helper.Log2(new LogType { LogFileName = "DpmData", LogLevelType = "info", LogMsg = sendTxt });
            viewTxt = sendTxt;
            if (!IsConnect)
            {
                return false;
            }
            IsUpdateOk = UpdateArea(i, sendTxt, out string msg);
            Msg = msg;
            Log2Helper.Log2(new LogType { LogFileName = "DpmResult", LogLevelType = "info", LogMsg = $"{DateTime.Now}--{IsUpdateOk}" });
            return IsUpdateOk;
        }

        /// <summary>
        /// 格式字符串约定$为换行符，每行12个汉字，第一行和第二行为时间和日期。第三行开始为内容
        /// </summary>
        /// <param name="txt"></param>
        /// <returns></returns>
        private string FormatStr(string txt)
        {
            ...
            txt = $"{DateTime.Now:yyyy年MM月dd日}${DateTime.Now:HH：mm：ss} {xq}${txt}";
            string sbc = NumberToSBC(txt);
            string[] arr = sbc.Split('$');
            string result = string.Empty;
            foreach (string line in arr)
            {
                result += line.PadRight(12, '　');
            }
            return result;
        }
        ...
        /// <returns></returns>
        private string NumberToSBC(string str)
        {
            str = str.Replace("0", "０").Replace("1", "１").Replace("2", "２").Replace("3", "３").Replace("4", "４").Replace("5", "５").Replace("6", "６").Replace("7", "７").Replace("8", "８").Replace("9", "９").Replace(" ", "　").Replace(":", "：");
            while (str.Length < 12)
            {
                str += "　";
            }
        ...
        /// 控制卡ip
        /// </summary>
        private bool MK1Connect(string ip, string port = "5005")
        {
            byte[] led_ip = Encoding.ASCII.GetBytes(ip);
            ...
            uint hand = Led5kSDK.CreateClient(led_ip, led_port, (Led5kSDK.bx_5k_card_type)card_type_list[type], 1, 1, null);
            m_dwCurHand = hand;
            return hand != 0;
        }
        ...
        /// 更新动态区参数设置
        /// </summary>
        /// <param name="AreaX">x轴坐标</param>
        /// <param name="AreaY">y轴坐标</param>
        /// <param name="AreaWidth">宽度</param>
        /// <param name="AreaHeight">宽度</param>
        /// <param name="Lines_sizes">行间距</param>
        /// <param name="AreaText">显示文本</param>
        public void UpdateAreaParameter(string AreaX, string AreaY, string AreaWidth, string AreaHeight, string Lines_sizes, string areaText)
        {
            bx_5k.AreaType = 0x06;
            //x轴坐标
            bx_5k.AreaX = (ushort)(Convert.ToUInt16(AreaX) / 8);
            //y轴坐标
            bx_5k.AreaY = Convert.ToUInt16(AreaY);
            bx_5k.AreaWidth = (ushort)(Convert.ToUInt16(AreaWidth) / 8);
            bx_5k.AreaHeight = Convert.ToUInt16(AreaHeight);
            bx_5k.Lines_sizes = Convert.ToByte(Lines_sizes);

            byte[] RunMode_list = new byte[3];
            RunMode_list[0] = 0;
            RunMode_list[1] = 1;
            RunMode_list[2] = 2;
            //运行模式 0 是自动循环   1是完成后停留再最后一页
            int rl = 1;
            ...
            //显示文本
            AreaText = System.Text.Encoding.Default.GetBytes(areaText);
        ...
        /// </summary>
        /// <param name="i"></param>
        public bool UpdateArea(uint i, string txt, out string msg)
        {
            UpdateAreaParameter(areaX, areaY, areaWidth, areaHeight, lines_sizes, txt);
            bx_5k.DynamicAreaLoc = (byte)i;
            int x = Led5kSDK.SCREEN_SendDynamicArea(m_dwCurHand, bx_5k, (ushort)bx_5k.DataLen, AreaText);
            if (x == 0)
            {
                msg = $"Y--{DateTime.Now:yyyy-MM-dd HH:mm:ss}更新屏幕成功，更新的内容为：{txt}{Environment.NewLine}";
                return true;
            }
            else
            {
                msg = $"N--{DateTime.Now:yyyy-MM-dd HH:mm:ss}更新屏幕失败，待更新的内容为：{txt}{Environment.NewLine}";
                return false;
            }
        }
                ...
                if (n > 0 && str_Area[n].Length > 1)
                {
                    if (str_Area[n].Substring(0, 2).Equals("FK"))
                    {
                        ...
                        if (str_Area[n].Length > 5)
                        {
                            ...
                            {
                                (Korean[(k * 2) + 1], Korean[k * 2]) = (Korean[k * 2], Korean[(k * 2) + 1]);
                            }
